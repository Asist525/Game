...
class PPOPolicy:
    ...
    def act_train(
        ...
        print(f"[Entropy] policy_entropy = {entropy:.4f}")
...
def save_checkpoint_policy(
    ...
    ckpt_path = os.path.join(checkpoint_dir, f"policy_epoch_{epoch:06d}_v3.pt")
    policy.save(ckpt_path)

    # 오래된 것 정리
    ckpts = sorted(Path(checkpoint_dir).glob("policy_epoch_*_v3.pt"))
...
def main_train():
    ...
    - 최종 learner policy를 'shared_policy_v3.pt'로 저장
    """
    config = PPOConfig(
        gamma=0.99,
        gae_lambda=0.95,
    ...
    # 최종 learner 정책 저장
    weight_path = "shared_policy_v3.pt"
