...
KILL_BONUS   = 1.0      # 돌 한 개 더 따면 +1
STEP_PENALTY = 0.005    # 턴당 -0.005 (200수 버티면 -1)
...
def save_checkpoint_policy(
    ...
    ckpt_path = os.path.join(checkpoint_dir, f"policy_epoch_{epoch:06d}_v2.pt")
    policy.save(ckpt_path)

    # 오래된 것 정리
    ckpts = sorted(Path(checkpoint_dir).glob("policy_epoch_*_v2.pt"))
...
def append_epoch_log(
    ...
    num_players: int,
    log_path: str = "training_metrics_v2.csv",
) -> None:
    """
...
def train_league_selfplay(
    ...
    보상 설계(B안):
      매 learner 턴마다
        r_t = KILL_BONUS * Δalive_t - STEP_PENALTY
      마지막에는 기존처럼 최종 alive_diff 를 한 번 더 더해줌.
    """
    if config is None:
        config = PPOConfig()
    ...
    for epoch in range(num_epochs):
        ...
        for ep in range(episodes_per_epoch):
            ...
            while not done:
                ...
                # ----- learner 턴 -----
                if learner_turn:
                    # 이 턴 직전 alive_diff (learner 관점)
                    alive_before = compute_terminal_reward(
                        obs,
                ...
                # ----- learner 턴에 대한 shaping reward -----
                if learner_turn:
                    alive_after = compute_terminal_reward(
                        obs,
                        my_color=learner_color,
                    )
                    delta_alive = alive_after - alive_before  # Δalive_t

                    shaped_r = KILL_BONUS * delta_alive - STEP_PENALTY
                    ep_rewards.append(float(shaped_r))

            # ---- 에피소드 종료: learner 입장에서 최종 리워드 & 승패 계산 ----
            # 최종 alive_diff를 terminal 보상으로 한 번 더 부여
            ...
            ep_reward_sum += R_learner   # 최종 승패 기준 요약에는 여전히 terminal만 사용
            ep_step_sum += step
...
def main_train():
    ...
    learner, league = train_league_selfplay(
        num_epochs=10000000000,  # 충분히 크게 잡기
        ...
        max_checkpoints=20,
        log_path="training_metrics_v2.csv",
    )

    # 최종 learner 정책 저장
    weight_path = "shared_policy_v2.pt"
