...
class PPOConfig:
    ...
    clip_coef: float = 0.2
    ent_coef: float = 0.01      # 엔트로피 계수 약간 상향 (0.005 -> 0.01)
    vf_coef: float = 0.5
...
def append_epoch_log(
    ...
    if not file_exists:
        with open(log_path, "w", encoding="utf-8") as f:
            f.write(
                "epoch,episodes,wins,draws,losses,win_rate,avg_alive_diff,avg_steps,learner_rating,num_players\n"
            )
...
def append_reward_breakdown_log(
    ...
    if not file_exists:
        with open(log_path, "w", encoding="utf-8") as f:
            f.write(
                "epoch,episodes,avg_step_penalty,avg_alive_trade,avg_shaping,avg_game_reward,total_avg_reward\n"
            )
    ...
    with open(log_path, "a", encoding="utf-8") as f:
        f.write(
            ...
            f"{avg_game_reward:.6f},{total_avg_reward:.6f}\n"
        )
    ...
    for epoch_idx in range(start_epoch + 1, start_epoch + num_epochs + 1):
        ...
        for ep in range(episodes_per_epoch):
            ...
            while not done:
                ...
                # base/alive 기준 상태
                alive_before = compute_alive_diff(obs, my_color=learner_color)
...
def main_train():
    ...
    config = PPOConfig(
        ...
        gae_lambda=0.95,
        clip_coef=0.2,
        ent_coef=0.01,   # 수정안 반영: 엔트로피 계수 상향
        vf_coef=0.5,
        max_grad_norm=0.5,
        learning_rate=1e-4,
        update_epochs=3,
